document.addEventListener('DOMContentLoaded', () => {
  ...
  const siteSegments = window.location.pathname.split('/').filter(Boolean);
  const musadaIndex = siteSegments.lastIndexOf('musada2');
  let baseUrl = './';

  if (musadaIndex !== -1) {
    const depthFromRoot = Math.max(siteSegments.length - musadaIndex - 1, 0);
  ...
  fetch(baseUrl + 'assets/data/data-pegawai.json')
    .then(response => {
    ...
    .then(data => {
      ...
      data.forEach(pegawai => {
        ...
        const foto = normalizeUrl(pegawai.foto || '');
        card.innerHTML = `
          <div class="pegawai-photo-col">
            <img src="${foto}" alt="${pegawai.nama}" class="pegawai-photo">
