...
class MessageRenderer {
   ...
   */
  renderMessages(messages, currentUser = { type: "user" }) {
   ...
   */
  addMessage(message, currentUser = { type: "user" }) {
   ...
   */
  addMessages(messages, currentUser = { type: "user" }) {
    if (!messages || messages.length === 0) {
      return;
    }
  ...
  addSystemMessage(html) {
    ...
    sys.innerHTML = DOMPurify.sanitize(html, { ALLOWED_TAGS: ['i'], ALLOWED_ATTR: ['class'] });
    ...
    if (message.attachment) {
      ...
      if (isPdf) {
        ...
        if (cached) {
          ...
          const urlParams = new URLSearchParams({ action: 'getFile', fileUrl: rawUrl });
          fetch(`${CONFIG.GAS_URL}?${urlParams.toString()}`)
            .then(res => res.json())
            .then(data => {
