...
class MessageStore {
  ...
  upsertMessage(message) {
    ...
    if (serverId && this.messagesById.has(serverId)) {
      ...
      this._emit("messageUpdated", { old: existing, new: merged });
      return merged;
    }
    ...
    if (clientId && this.messagesByClientId.has(clientId)) {
      ...
      if (serverId && serverId !== clientId) {
        ...
        const upgraded = { ...optimistic, ...message, id: serverId };
        ...
        this._emit("messageUpdated", { old: optimistic, new: upgraded });
        return upgraded;
      } else {
        ...
        this._emit("messageUpdated", { old: optimistic, new: merged });
        return merged;
      }
    }
  ...
  updateMessage(messageId, updates) {
    ...
    if (changed) {
      this._emit("messageUpdated", { old: before, new: message });
    }
